...
vetor_Psi_cplx = np.zeros(Q,dtype = np.complex64)
vetor_Psi_real, vetor_Psi_imag = np.zeros(Q), np.zeros(Q)
...
for i in range(samples):
	...
	vetor_Psi_cplx += analyzer.get_soma_psi_cplx()
	vetor_Psi_real += analyzer.get_soma_psi_real()
	vetor_Psi_imag += analyzer.get_soma_psi_imag()
...
# filenome3 = 'Psi_hex_complex_n0'+str(n0)+'_eta'+str(int(1000*eta))+'_t'+str(Q)+'_S'+str(samples)+'_p'+str(per)+'.csv'
# arquivo3 = open(filenome3, 'w') #exportar a configuracao inicial
# for i3 in np.arange(0,Q,per):
# 	arquivo3.write(str(tempo[i3])+' , '+str(vetor_Psi_cplx[i3])+' , '+str(vetor_Psi_real[i3])+' , '+str(vetor_Psi_imag[i3])+'\n')
# arquivo3.close() 

filenome3 = 'Psi_hex_complex_n0'+str(n0)+'_eta'+str(int(1000*eta))+'_t'+str(Q)+'_S'+str(samples)+'_p'+str(per)+'.csv'
arquivo3 = open(filenome3, 'w') #exportar a configuracao inicial
for i3 in np.arange(0,Q,per):
	arquivo3.write(str(tempo[i3])+' , '+str(vetor_Psi_real[i3])+' , '+str(vetor_Psi_imag[i3])+'\n')
arquivo3.close() 
